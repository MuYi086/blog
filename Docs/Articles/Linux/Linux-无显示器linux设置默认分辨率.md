...
::: warning 注意
`Identifier` ：虚拟显示器的名称，你可以根据需要自定义。
...
# 使用nvidia-xconfig生成xorg配置
sudo nvidia-xconfig
# 编辑xorg.conf文件，将 Virtual 2560 1440 加入到 Section "Screen" 中
# 保存，重启

# 方法二:
# 使用nvidia-xconfig命令加参数生成xorg配置
sudo nvidia-xconfig --use-display-device=all --virtual=2560x1440
```

::: warning 注意
使用 `nvidia-xconfig` 配置虽然简单，但是会导致显示无法显示底部菜单（本人使用 `deepin23` 测试，其他系统可能也会出现类似问题）
:::
...
1. [解决Todesk、向日葵等远程访问“无显示器”主机黑屏问题](https://cloud.baidu.com/article/3309953)
1. [X11 Dummy configuration.txt](https://gist.github.com/divinity76/ce210b5dbcd9ea7d0585ac403caef577)
1. [如何通过nvidia-xconfig开启默认显示器](https://wenku.csdn.net/answer/4796c3904fac4287b6bca7e60179984c)