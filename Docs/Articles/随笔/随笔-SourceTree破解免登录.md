...
1. 初次运行安装 `SourceTree` 弹出如图,然后退出
  ![安装](/Images/随笔/随笔-SourceTree免登录/install.jpg '安装')
...
1. 重新执行安装步骤,然后选择 `我不想使用Mercurial`
    ...
    ![不使用Mercurial](/Images/随笔/随笔-SourceTree免登录/choose.jpg '不使用Mercurial')

1. 也可以在进入软件主界面中的设置配置 `ssh` 客户端

    ![配置ssh客户端](/Images/随笔/随笔-SourceTree免登录/ssh_config.png '配置ssh客户端')
