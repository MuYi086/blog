...
## 前言
本文是对前文 [科学上网-让你连接互联网](/Docs/Linux/Shell/科学上网-让你连接互联网) 的使用总结，同时增加了心心念念的 `v2ray` 部分。在这之前服务器环境都已搭建和配置好,本文仅展示梯子的使用，且不讨论 `tor` , `xx-net` 等其他方式。
...
1. `Windows` 下载: [地址](https://sourceforge.net/projects/shadowsocks-heroku/)

    ```shell
    https://sourceforge.net/projects/shadowsocks-heroku/
    ```
1. `Linux / Mac` 下载: [地址](https://github.com/VincentChanX/shadowsocks-over-websocket.git)
...
## 浏览器代理
最热门的代理扩展莫过于 `SwitchyOmega`  [下载](https://github.com/FelisCatus/SwitchyOmega/releases)
```shell
